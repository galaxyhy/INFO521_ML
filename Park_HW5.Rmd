...
college.lm <- lm(Apps ~ Grad.Rate + Outstate + Top10perc + Expend, data = train.College)
summary(college.lm) # residual standard error is 13.65

y_hat_train <- predict(college.lm, data = train.College)
train_MSE <- mean((train.College$Grad.Rate - y_hat_train)^2)

ggplot(train.College, aes(x = Apps, y=Grad.Rate)) +
    geom_point() + 
    geom_smooth(method = "lm") +
...
# ridge regression
matrix_mod <- model.matrix(Apps ~ Grad.Rate + Outstate + Top10perc + Expend, data = train.College)
...
mean((ridge_pred - test.College$Apps)^2) # Calculate test MSE which was 900285.5
...
lasso_coef <- predict(out, type = "coefficients", s = bestlam)[1:8,] # Display coefficients using lambda chosen by CV
lasso_coef
