...
sample_size <- round(nrow(Boston)*.70) # setting what is 70%
index <- sample(seq_len(nrow(Boston)), size = sample_size)
 
train.Boston <- Boston[index, ]
test.Boston <- Boston[-index, ]

## logistic regression
logit.fit <- glm(crim ~ ., data = train.Boston, family = binomial)
summary(logit.fit)
logit.probs <- predict(logit.fit, newdata = test.Boston, type = "response")
pred_values <- rep(0, length(test.Boston$crim))
pred_values[logit.probs > 0.5] <- 1

table(Predicted = pred_values, Survived = test.Boston$crim)
# Error: 
mean(test.Boston$crim != pred_values)

## boosting
boost.boston <- gbm(crim~., data=train.Boston, distribution = "gaussian", n.trees = 5000, shrinkage = 0.01, interaction.depth = 4) 
...
# yhat boosted model
yhat.boost <- predict(boost.boston, newdata=train.Boston, n.trees = 5000)
...
set.seed(1)
bag.boston <- randomForest(crim ~., data = train.Boston, mtry = 12, importance = TRUE)
yhat.bag <- predict(bag.boston, newdata = test.Boston)
plot(yhat.bag, boston.test)
abline(0,1)
mean((yhat.bag - boston.test)^2)

bag.boston <- randomForest(crim ~., data = train.Boston, mtry = 12, ntree=25)
yhat.bag <- predict(bag.boston, newdata = test.Boston)
mean((yhat.bag - boston.test)^2)

set.seed(1)
rf.boston <- randomForest(crim ~ . , data = train.Boston, mtry = 6, importance=TRUE)
yhat.rf <- predict(rf.boston, newdata = test.Boston)
...
The testing error is lower for the random forest model versus logistic regression or boosting.
...
# subset data for numerical
flights_num <- select_if(flights2, is.numeric)

# look for NAs
colsums(is.na(flights2))
flights2[rowSums(is.na(flights2)) > 0, ]   
mice_plot <- aggr(flights_num, col=c('blue', 'yellow'), numbers=TRUE, sortVars=T)

# impute missing values
library(Hmisc)
flights2$arr_delay_imp <- with(flights2, impute(arr_delay, mean))
flights2$delay_imp <- with(flights2, impute(delay, mean))
flights2$dep_time_imp <- with(flights2, impute(dep_time, mean))

# training/testing split - mostly repeating the provided code
set.seed(5)
...
flights_train %>% map(mean)

flights_train %>% map_dbl(mean, na.rm = TRUE)
summary(flights_train)
...
flights_train %>%
  group_by(origin) %>%
  na.omit() %>% 
  summarise(mean_ad = mean(arr_delay), sd_ad = sd(arr_delay), n_arr =n())
# plot dep delay by origin
ggplot(data= flights_train, aes(x= origin, y= dep_delay, fill = origin)) +   
  geom_violin()
  
ggplot(data= flights_train, aes(x= origin, y= arr_delay, fill = origin)) +   
  geom_violin()
